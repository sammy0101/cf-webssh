...
Generated on: Wed Jun 24 16:56:51 UTC 2026
...
<body class="bg-slate-950 text-slate-100 font-sans min-h-screen">
  ...
  <script>
      ...
      sftpWs.onmessage = async (event) => {
        ...
        try {
          ...
          // A. 檔案清單渲染
            ...
            updateBreadcrumbs(sftpCurrentPath); // 動態更新麵包屑導航條
    ...
    // 6.3.5 新增「類 WINDOWS」路徑麵包屑動態渲染函數 (新增)
    function updateBreadcrumbs(path) {
      ...
      parts.forEach((part, index) => {
        ...
        const targetPath = currentBuildPath; // 區塊變數固定

        const partSpan = document.createElement('span');
        // 若為最後一個節點（目前資料夾），高亮但不具備跳轉點擊功能
