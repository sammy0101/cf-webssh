...
<body class="bg-slate-950 text-slate-100 font-sans min-h-screen">
  ...
  <div id="terminal-screen" class="fixed inset-0 bg-black hidden flex-col z-50">
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div id="sftp-sidebar" class="w-80 bg-slate-900 border-l border-slate-800 flex flex-col hidden transition-all duration-300">
        ...
        <!-- 操作控制與上傳按鈕 -->
        <div class="p-2 border-b border-slate-800 bg-slate-900/50 flex items-center gap-1.5 justify-between">
          <div class="flex items-center gap-1.5">
            <button onclick="sftpGoUp()" class="bg-slate-800 hover:bg-slate-700 text-slate-300 px-2 py-1 rounded text-xs font-medium" title="返回上層目錄">
              向上 ↩
            </button>
            <button onclick="refreshSftpList()" class="bg-slate-800 hover:bg-slate-700 text-slate-300 px-2 py-1 rounded text-xs font-medium" title="重新整理列表">
              整理 ↻
            </button>
          </div>
          <!-- 收納整合後的手動上傳按鈕 -->
          <button onclick="triggerFileInput()" class="text-xs bg-emerald-600 hover:bg-emerald-500 text-white px-2.5 py-1 rounded font-medium transition flex items-center gap-1">
        ...
        <div class="p-2 border-b border-slate-800 bg-slate-950 flex items-center gap-1">
          <input type="text" id="sftp-current-path" readonly class="w-full bg-transparent border-none text-slate-300 text-[11px] font-mono select-all focus:outline-none">
        </div>
  ...
  <script>
    ...
    // 6. 建立單一 SFTP WebSocket 安全連線 (核心優化)
