...
Generated on: Tue Jun 23 18:40:03 UTC 2026
...
<body class="bg-slate-950 text-slate-100 font-sans min-h-screen">
  ...
  <script>
    ...
    function handleSftpUpload(connectionId, file) {
      ...
      const chunkSize = 64 * 1024; // 設定 64KB 為分塊基準
      let offset = 0;

      currentSftpWs.onopen = () => {
        // 連線建立，發送上傳中繼資料 (Metadata)
            ...
            if (offset < file.size) {
              ...
              reader.onload = (e) => {
                if (currentSftpWs && currentSftpWs.readyState === WebSocket.OPEN) {
                  currentSftpWs.send(e.target.result); // 發送二進位封包
                }
              };
              reader.readAsArrayBuffer(slice);
            } else {
              // 檔案全數送完，發送結尾指令以關閉遠端檔案寫入
